...
def server_start():
    ...
    runtime_dll = []
    load_module = tvm.get_global_func("tvm.rpc.server.load_module")
    ...
    def program_fpga(file_name):
        ...
        if env.TARGET == "pynq":
            ...
            logging.info("Load the de10pro program function")
            load_vta_dll()
        else:
            logging.info("No program function loaded")
    ...
    def reconfig_runtime(cfg_json):
        ...
        """
        print("reconfig_runtime...")
        env = get_env()
        print( 'cfg_json:={0}'.format(cfg_json))
        if runtime_dll:
