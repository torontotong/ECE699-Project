...
struct DataBuffer {
  ...
  void MemCopyToHost(void* dst, const void* src, size_t size) { 
      printf("Runtime::MemCopyToHost(), from 0x%llx to 0x%llx for %d bytes\n", src, dst, size);
  ...
  static DataBuffer* Alloc(size_t size) {
    ...
    printf("Runtine::Alloc(): Allocated fpga buffer 0x%llx from VTAMemAlloc() \n", data);
    CHECK(data != nullptr);
    DataBuffer* buffer = new DataBuffer();
    buffer->data_ = data;
    buffer->phy_addr_ = VTAMemGetPhyAddr(data);
    printf("Runtine::Alloc(): Get physical address of fpga buffer 0x%llx from VTAMemGetPhyAddr(0x%llx) \n", buffer->phy_addr_ , data);
...
class BaseQueue {
 public:
  ...
  void InitSpace(uint32_t elem_bytes, uint32_t max_bytes, bool coherent, bool always_cache) {
    ...
    printf("Runtine::InitSpace(): Allocated fpga buffer 0x%llx from VTAMemAlloc() \n", fpga_buff_);
    CHECK(fpga_buff_ != nullptr);
    fpga_buff_phy_ = VTAMemGetPhyAddr(fpga_buff_);
    printf("Runtine::InitSpace(): Get physical address of fpga buffer 0x%llx from VTAMemGetPhyAddr(0x%llx) \n", fpga_buff_phy_ , fpga_buff_);
  }
  /*!
 ...
 public:
  ...
  void FlushUopLoad(VTAMemInsn* insn) {
    if (sram_begin_ != sram_end_) {
      ...
      printf("Runtime::FlushUopLoad Update cache idx to physical address map\n");
      printf("Change Dram_base_addr from 0x%llx to 0x%llx\n",insn->dram_base, (fpga_buff_phy_ + offset) / kElemBytes);
      insn->dram_base = (fpga_buff_phy_ + offset) / kElemBytes;
      insn->y_size = 1;
  ...
  void ReadBarrier() {
    ...
    printf("Runtime::ReadBarrier(1) merge all the cache entries and do CopyFromHost once\n");
    ...
    printf("Runtime::ReadBarrier(1) VTAMemCopyFromHost from host 0x%llx to device 0x%llx \n",lbuf, fpga_buff_ );
...
class InsnQueue : public BaseQueue<VTAGenericInsn> {
 public:
  ...
  void ReadBarrier() {
    ...
    printf("Runtime::ReadBarrier(2) Copy contents of DRAM buffer to FPGA buff\n");
    printf("Runtime::ReadBarrier(2) VTAMemCopyFromHost from host 0x%llx to device 0x%llx \n",dram_buffer_.data(), fpga_buff_ );
...
class CommandQueue {
 public:
  ...
  void LoadBuffer2D(void* src_dram_addr, uint32_t src_elem_offset, uint32_t x_size, uint32_t y_size,
    ...
    printf("Runtime::LoadBuffer2D GetElemBytes()=%d, dst_memory_type=%d\n",GetElemBytes(dst_memory_type), dst_memory_type );
    insn->dram_base = src->phy_addr() / GetElemBytes(dst_memory_type) + src_elem_offset;
    printf("Runtime::LoadBuffer2D Dram_base_addr 0x%llx\n",insn->dram_base);
    insn->y_size = y_size;
    insn->x_size = x_size;
    insn->x_stride = x_stride;
    printf("Runtime::LoadBuffer2D y_size 0x%x, x_size 0x%x, x_stride 0x%x\n",y_size, x_size, x_stride);
  ...
  void StoreBuffer2D(uint32_t src_sram_index, uint32_t src_memory_type, void* dst_dram_addr,
    ...
    printf("Runtime::StoreBuffer2D GetElemBytes()=%d, src_memory_type=%d\n",GetElemBytes(src_memory_type), src_memory_type );
    insn->dram_base = dst->phy_addr() / GetElemBytes(src_memory_type) + dst_elem_offset;
    printf("Runtime::StoreBuffer2D Dram_base_addr 0x%llx\n",insn->dram_base);
    //printf("Runtime::StoreBuffer2D Set Stored Buufer Virtual addr 0x%llx\n",dst->virt_addr());
    //VTASetStoreBufferVirAddr( dst->virt_addr() );
    insn->y_size = y_size;
    insn->x_size = x_size;
    insn->x_stride = x_stride;
    printf("Runtime::StoreBuffer2D y_size 0x%x, x_size 0x%x, x_stride 0x%x\n",y_size, x_size, x_stride);
    insn->y_pad_0 = 0;
    insn->y_pad_1 = 0;
    insn->x_pad_0 = 0;
    insn->x_pad_1 = 0;
  ...
  void Synchronize(uint32_t wait_cycles) {
    ...
    printf("Runtime::Synchronize Calling VTADeviceRun() device:0x%x, insn_queue:0x%llx, queue_count:0x%x, wait cycles:%ld\n",device_, insn_queue_.dram_phy_addr(), insn_queue_.count(), wait_cycles);
    int timeout =
        VTADeviceRun(device_, insn_queue_.dram_phy_addr(), insn_queue_.count(), wait_cycles);
    printf("Runtime::Synchronize called VTADeviceRun() returned timeout = %d\n", timeout);
    CHECK_EQ(timeout, 0);
    // Reset buffers
    uop_queue_.Reset();
    insn_queue_.Reset();
...
void VTABufferCopy(const void* from, size_t from_offset, void* to, size_t to_offset, size_t size,
  ...
  printf("Runtime::VTABufferCopy(), org_from is 0x%llx, org_to is 0x%llx \n", from , to);
  ...
  if (from_buffer) {
    ...
    printf("Runtime::VTABufferCopy(),from_buffer->virt_addr = from = 0x%llx, to is 0x%llx \n", from,to);
    ...
    printf("Runtime::VTABufferCopy(), from 0x%llx, to_buffer->virt_addr = to = 0x%llx\n", from,to);
