...
def _server_env(load_library, work_path=None):
    ...
    def program_fpga(file_name):
            ...
            #os.system("sudo mkdir /sys/kernel/config/device-tree/overlays/0")
            #os.system("sudo echo socfpga_ovl.dtbo > /sys/kernel/config/device-tree/overlays/0/path")
            #os.system("sleep 5")
            #os.system("sudo insmod ~/cma/cma.ko")
            logging.info("Program FPGA Done")
    ...
    def reconfig_runtime(cfg_json):
            ...
            print('tvm.contrib.vta.reconfig_runtime called with {0}'.format(cfg_json))
