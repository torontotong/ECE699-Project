 ...
 *
 * \file de10pro_mgr.h
 * \brief DE10-Pro fpga manager.
 */

#ifndef VTA_DE10PRO_DE10PRO_MGR_H_
#define VTA_DE10PRO_DE10PRO_MGR_H_
...
struct De10ProMgr {
  ...
  struct Reg32 {
    ...
    void print(const char *name, bool addr = false) {
      if (addr)
        printf("DE10-Pro-Mgr: %16s: 0x%08x addr: %p\n", name, read(), m_addr);
      else
        printf("DE10-Pro-Mgr: %16s: 0x%08x\n", name, read());
    }
  ...
  struct stat : public Reg32 {
    ...
    void print(bool addr = false, bool fields = true) {
      ...
      if (fields) {
        printf("DE10-Pro-Mgr: %16s: %x\n", "msel", msel);
        printf("DE10-Pro-Mgr: %16s: %s\n", "mode", mode_str());
      }
    }
  ...
  struct monitor {
    ...
    void done(bool status = true) {
      ...
      printf("\rDE10-Pro-Mgr: %-32s : %s in %u us\n", msg, rs, elapsed);
      if (!status) {
        m_status = false;
        throw 1;
      }
    ...
    ~monitor() {
      ...
      printf("DE10-Pro-Mgr: EXIT %s in %u us\n", rs, elapsed);
    }
 ...
 public:
  ...
  bool program_rbf(const char *rbf) {
    ...
    int rbf_fd;
    uint32_t count = 0;
    printf("DE10-Pro-Mgr: Programming FPGA from image %s\n", rbf);
    ...
    try {
      ...
      if (stat.msel_is_invalid()) {
        printf("DE10-Pro-Mgr: msel %x is not a valid HPS configuration\n", stat.msel);
      } else {
        ctrl.read();
      ...
      do {
        ...
        if (bytes > 0) {
          if (!(count % (1<<16))) {
            printf("\rDE10-Pro-Mgr: %-32s : %u B", mon.msg, count);
            fflush(stdout);
          }
          data.write();
          count += bytes;
      ...
      mon.done(count > 0);
      printf("DE10-Pro-Mgr: %-32s : written %u B\n", mon.msg, count);
      close(rbf_fd);
...
#endif  // VTA_DE10PRO_DE10PRO_MGR_H_
